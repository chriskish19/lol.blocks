...
std::vector<DirectX::XMFLOAT3> lol_blocks_api::cube_to_vb(const lb::cube& cb) {
    std::vector<DirectX::XMFLOAT3> vertices;

    for (int face = 0; face < lb::cube::size; ++face) {
        for (int tri = 0; tri < lb::square::size; ++tri) {
...
lb::cube lol_blocks_api::create_cube(lb::size sz, lb::position p)
{
    lb::size_f wc = sz.convert_wc();
    lb::cube cb = {};
...
std::wstring lol_blocks_api::to_wide_string(const std::string& narrow)
{
    /*
...
std::string lol_blocks_api::to_narrow_string(const wchar_t* wide)
{
...
std::string lol_blocks_api::to_narrow_string(const std::wstring& wide)
{
   /*
   *
   Converts a sequence of wide characters from the array whose first element is pointed to by *src
