...
LRESULT win32gui::pop_up_window::ClassWindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) noexcept
{
    switch (uMsg) {
    case WM_DESTROY:
        PostQuitMessage(0);
...
win32gui::pop_up_window::pop_up_window(const std::wstring& title) noexcept
    :m_title(title)
{
    create_pop_up_window();
}

win32gui::pop_up_window::pop_up_window() noexcept
{
    create_pop_up_window();
}
